...
{
  //Constructor. Creates an empty queue. Note: constructors are actually declared and implemented in the derived classes, not in the base class.
  front = NULL;
  back = NULL;
}
template <typename DataType>
QueueLinked<DataType>::QueueLinked(const QueueLinked& other)
{
  ...
  QueueNode* otherPtr = other.front; //QueueNode* otherPtr = other.head;
  while(otherPtr != NULL)
  {
    enqueue(otherPtr->dataItem);
