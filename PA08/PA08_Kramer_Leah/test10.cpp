...
unsigned int TestData::hash(const string& str) { //adds ascii values of characters from string
    unsigned int val = 0;
...
int main(int argc, char **argv) {
    HashTable<TestData, string> table(7); //Creates hash table


    print_help();

    do {
	table.showStructure(); //prints table
        // HashTable<TestData, string> copyTable(table);
        //copyTable.showStructure();

	cout << endl << "Command: ";
	char cmd;   //Reads command
	cin >> cmd;

	TestData item;
	if (cmd == '+' || cmd == '?' || cmd == '-') { //if cmd is + ? or -
	    string key;
	    cin >> key; //following is string to be read in
	    item.setKey(key); //key is set for TestData class object item
	}
