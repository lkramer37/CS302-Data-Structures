...
class Garden{
  public:
    //!Default Constructor - Creates a 40x40 array and sets each element to 'x'. Creates an array to hold the values for patch sizes. Initializes the number of patches to 0.
    Garden(); 
    //!readIn Function - Reads from terminal into gardenArray
    void readIn(int r, int c); 
    //!countPatches Function - Uses dimension parameters to locate the number of patches in gardenArray.
    int countPatches(int row, int col);
    //!printPatchArray Function - Calls the sort function, then prints the patchSize values to the terminal
    void printPatchArray();
    //!sortPatchArray Function - Sorts patchArray from smallest to largest
    void sortPatchArray(); 
    //!findPatch Function - finds a patch and returns it's size
